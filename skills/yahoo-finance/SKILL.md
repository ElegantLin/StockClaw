...
This repo ships a local executable script at `{baseDir}/yf.py`.

- For historical backtest preparation, prefer:
  `uv run {baseDir}/yf.py history AAPL.US MSFT.US --start 2026-03-09 --end 2026-03-12 --output json`
- For quick spot checks, prefer:
  `uv run {baseDir}/yf.py price AAPL.US --output json`
- For broader multi-factor stock analysis, prefer the `stock-analysis` skill.
